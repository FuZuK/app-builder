...
function Builder({
	...
	function makeProjectCopy() {
		return new Promise((res, rej) => {
			exec(`mkdir -p ${ copyPath }`, (e, stdout, stderr) => {
				if (e) rej(e);
				else {
					exec(`cp -R ${ project }/* ${ copyPath }`, e => {
						if (e) rej(e);
						else res();
					});
				}
	...
	function buildApp() {
		return new Promise((res, rej) => {
			const prc = exec(`./gradlew ${ assembleTask }`, {
				cwd : copyPath
			}, e => {
				if (e) rej(e);
				else res();
	...
	function moveApk() {
		return new Promise((res, rej) => {
			exec(`mv ${ srcApkPath } ${ destination }`, e => {
				if (e) rej(e);
				else res();
			});
		});
